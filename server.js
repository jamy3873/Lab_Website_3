...
app.get('/player_info/select_player', function(req, res) {
  ...
  var players = 'select id, name from football_players;';
  var chosen_player = "select * from football_players where id='"+playerID+"';";
  var game_count = "select count(*) from football_games where (select id from football_players where id='"+playerID+"')=any(players);";
  db.task('get-everything', task => {
        return task.batch([
            task.any(players),
